...
//	public void incrementCountEcplored() {
//		for (int i = 0; i < rows; i++) {
//			for (int j = 0; j < cols; j++) {
...
//	}
	...
	public List<Tile> getNeighbour(Tile t) {
		List<Tile> list = new LinkedList<Tile>();
		for (Aim a : Aim.values()) {
			if (a != Aim.DONTMOVE)
				list.add(getTile(t, a));
		}
		return list;
	}
	 ...
	 * @return
	 */
	public List<Tile> getMoveAbleNeighbours(Tile t) {
		List<Tile> list = new LinkedList<Tile>();
		for (Aim a : Aim.values()) {
	...
	public void clearMyAnts() {
		for (Ant ant : myAnts) {
			Tile position = ant.getAntPosition();
	...
	public void clearFood() {
		for (Tile food : foodTiles) {
			map[food.getRow()][food.getCol()].setType(Ilk.LAND);
		}
		foodTiles.clear();
	}
