...
public class Pathfinding {
	...
	public Pathfinding(Logger logger, GameInformations ants) {
		this.logger = logger;
		this.gameI = ants;
	}
	...
	public Set<Tile> visitableInXSteps(Tile postion, int x) {
		...
		while (!q.isEmpty()) {
			...
			for (Tile next : gameI.getNeighbour(tmp)) {
				// increase pathcost
	...
	public boolean searchShortestPath(Tile start, Tile end, List<Tile> graph,
		...
		while (!kl.isEmpty()) {
			...
			for (Tile i : gameI.getNeighbour(tmp)) {
				if (graph.contains(i)) {
					// logger.log("drinne");
		...
		public List<Tile> aStar(Tile source, Tile target, int weight) {
			...
			while (openSet.size() > 0) {
				...
				if (current.getId().equals(target.toString())) {
					...
					List<Tile> neighbors = gameI.getMoveAbleNeighbours(current.getNode());
